...
class StaminaAkkaAserializerWithLowLevelPersisterApiSpec extends WordSpecLike with Matchers with OptionValues with TryValues with Inside with Inspectors {
  import TestDomainPersistence._

  val serializer = new StaminaAkkaSerializer {
    val persister = TestDomainPersister
  }

  import serializer._

  "The StaminaAkkaSerializer (using spray-json)" should {
...
object TestDomainPersistence extends TestDomain {
  import json._

  val TestDomainPersister = Persister(
    toPersisted = {
